...
uint64_t expected_nodes_position_4[] = {
    1ULL, 6ULL, 264ULL, 9467ULL, 422333ULL, 15833292ULL,
};
...
uint64_t expected_nodes_position_6[] = {
    1ULL, 46ULL, 2079ULL, 89890ULL, 3894594ULL, 164075551ULL,
};
...
PerftTest test_suite[]
    = { {
        ...
        },
        {
            ...
            .fen = "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/"
                   "R3K2R w KQkq -",
        ...
        {
            .name = "Position 4",
            .fen = "r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/"
                   "R2Q1RK1 w kq - 0 1",
        ...
        {
            .name = "Position 6",
            .fen = "r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/"
                   "1PP1QPPP/R4RK1 "
                   "w - - 0 10",
...
void run_perft_test_suite(void)
{
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        if (!parsedFen) {
            printf("Failed to parse FEN for test '%s'. Skipping this "
                   "test.\n\n",
                   test.name);
            total_failed
                += test.max_depth + 1; // Count all depths as failed for this test
            continue;
        }
        ...
        for (int depth = 1; depth <= test.max_depth; depth++) {
            ...
            printf("Depth %d: %llu (%.3fs, %.0f nodes/sec)", depth, nodes, elapsed,
