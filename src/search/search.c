...
static bool should_stop_search(SearchContext* ctx)
{
    ...
    if ((nodes & 2047) == 0) {
        long long hard_deadline
            = atomic_load(&ctx->control->hard_deadline_ms);
...
void search_run(const SearchInput* input, SearchControl* control)
{
    ...
    while (current_depth <= max_depth) {
        ...
        long long current_soft_limit
            = atomic_load(&control->soft_limit_ms);
        ...
        if (!atomic_load(&control->pondering)
                        ...
                        ScoredMove* scored_moves, Move tt_move, int ply)
{
    ...
    for (int i = 0; i < move_list->count; i++) {
        ...
        if (tt_move != MOVE_NONE && curr_move == tt_move) {
            score = TT_MOVE_SCORE;
        } else if (move_is_capture(board, curr_move)
            ...
            if (mvv_lva >= 0 || move_is_promotion(curr_move)) {
                ...
                score = BAD_CAPTURE_BASE_SCORE + mvv_lva + promo_bonus;
                   ...
                   && curr_move == ctx->killer_moves[ply][0]) {
