...
magic build="dev": dirs
    cc="${CC:-cc}"; uname_s="$(uname -s)"; uname_m="$(uname -m)"; archflags=""; if [ "$uname_s" = "Darwin" ] && [ "$uname_m" = "arm64" ]; then archflags="-mcpu=native"; fi; case "{{build}}" in debug) mode_cflags="-O0 -g3 -fno-omit-frame-pointer"; mode_ldflags="" ;; release) mode_cflags="-O3 -DNDEBUG -flto"; mode_ldflags="" ;; *) mode_cflags="-O3 -g1 -DNDEBUG"; mode_ldflags="" ;; esac; git_version="$(git describe --tags --always --dirty 2>/dev/null || printf '%s' dev)"; cflags="-std=c17 -Wall -Wextra ${archflags} ${mode_cflags}"; $cc -I src -DVERSION="\"${git_version}\"" $cflags $mode_ldflags src/attacks/magic_gen.c src/attacks/sliding_attacks.c -o magic_gen
    ./magic_gen

# Build and run perft executable.
perft_test build="dev": dirs
...
archive build="dev":
    just all "{{build}}"
    git_version="$(git describe --tags --always --dirty 2>/dev/null || printf '%s' dev)"; artifact_dir="artifacts/${git_version}"; mkdir -p "$artifact_dir"; cp prophet "$artifact_dir/prophet-{{build}}"; printf 'Archived %s\n' "$artifact_dir/prophet-{{build}}"

# Build perft target and archive a versioned artifact for historical tracking.
archive-perft build="dev":
    just perft_test "{{build}}"
    git_version="$(git describe --tags --always --dirty 2>/dev/null || printf '%s' dev)"; artifact_dir="artifacts/${git_version}"; mkdir -p "$artifact_dir"; cp perft "$artifact_dir/perft-{{build}}"; printf 'Archived %s\n' "$artifact_dir/perft-{{build}}"

# Build magic generator and archive a versioned artifact for historical tracking.
archive-magic build="dev":
    just magic "{{build}}"
    git_version="$(git describe --tags --always --dirty 2>/dev/null || printf '%s' dev)"; artifact_dir="artifacts/${git_version}"; mkdir -p "$artifact_dir"; cp magic_gen "$artifact_dir/magic_gen-{{build}}"; printf 'Archived %s\n' "$artifact_dir/magic_gen-{{build}}"
...
# Full release flow from main: ensure clean tree, promote dev, build+archive release binary, create+push tag.
release ver:
    just require-clean
